...
const editOrganization = async (organization = { owner_id: '', name: '', image_url: '', id: '' }, client = null) => {
    ...
    const organizationQuery = `update ${DB_ORGANIZATIONS} set (name, image_url) = ($1,$2) where id = $3 returning *`
    let result;
    if (client) {
        result = await client.query(organizationQuery, [organization.name, organization.image_url, organization.id])
    } else {
        result = await query(organizationQuery, [organization.name, organization.image_url, organization.id])
    }
    return result
}
